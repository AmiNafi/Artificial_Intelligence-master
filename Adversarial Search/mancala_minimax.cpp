...
struct State {
	...
	void printState() {
		...
		cout << "(Store 1)" << endl;
		cout << "\t\t\t";
		for (int i = 0; i < 6; i++) cout << court[1][i] << " ";
		cout << endl;
		cout << "   " << store[1] << "\t\t\t\t\t\t\t" << store[0] << endl;
		cout << "\t\t\t";
		for (int i = 0; i < 6; i++) cout << court[0][i] << " ";
		cout << endl;
		cout << "(Store 2)-------";
		cout << "(Player 1)------";
		cout << "(Store 1)" << endl;
...
State move(State s) {
	...
	int allzero = 1;
	for (int i = 0; i < 6; i++) {
		if (s.court[s.player][i]) allzero = 0;
	}
	if (allzero) {
		for (int i = 0; i < 6; i++) {
...
vector < int > getChildList(int id, int player) {
	...
	for (int curpos = 0; curpos < 6; curpos++) {
		...
		if (s.court[player][curpos] == 0) continue;
...
bool gameover(int pos) {
	...
	for (int i = 0; i < allStates[pos].court[0].size(); i++) {
		if (allStates[pos].court[0][i]) over = 0;
	}
	for (int i = 0; i < allStates[pos].court[1].size(); i++) {
		if (allStates[pos].court[1][i]) over = 0;
	}
	return over;
}
